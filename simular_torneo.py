"""
Simula el avance del torneo hasta Cuartos de Final.

Qué hace, en orden:
1. Registra resultado a los 72 partidos de fase de grupos (ya existentes).
2. Calcula quién clasifica: los 2 primeros de cada grupo (24) + los 8 mejores
   terceros (comparando puntos, luego diferencia de gol, luego goles a favor).
3. Arma Dieciseisavos (16 partidos) con esos 32 clasificados, y les pone resultado.
4. Arma Octavos (8 partidos) con los 16 ganadores, y les pone resultado.
5. Arma Cuartos de Final (4 partidos) con los 8 ganadores, PERO SIN RESULTADO
   — quedan listos en el calendario para que el admin los complete en vivo
   durante la demo.

Todo se hace llamando a tu propia API (no toca la base de datos directo),
así que respeta toda tu lógica de negocio real: actualiza estadísticas de
equipos, dispara auditoría, intenta notificar a UTNGolCoin, etc.

Uso:
    python3 simular_torneo.py
"""
...
random.seed(42)  # para que los resultados simulados sean siempre los mismos
...
def get_qualifiers():
    print("=== 2. Calculando clasificados (2 primeros de cada grupo + 8 mejores terceros) ===")
    groups = get("/grupos")
    top2 = []
    thirds = []
    for g in groups:
    ...
    qualifiers = top2 + best_thirds
    print(f"  {len(qualifiers)} equipos clasificados a Dieciseisavos")
    return [q["idTeam"] for q in qualifiers], [q["name"] for q in qualifiers]
...
def get_venue_ids():
    venues = get("/sedes")
    return [v["idVenue"] for v in venues]
...
def main():
    ...
    if not login:
        print("No se pudo iniciar sesión como admin. Revisa que el hash de la contraseña ya esté corregido.")
        return
    print(f"Login OK, rol: {login.get('role')}\n")

    simulate_group_stage()
    qualifier_ids, qualifier_names = get_qualifiers()

    r32_winners = play_round(qualifier_ids, "DIECISEISAVOS", fifa_start=101, with_result=True)
    r16_winners = play_round(r32_winners, "OCTAVOS", fifa_start=121, with_result=True)
    play_round(r16_winners, "CUARTOS", fifa_start=129, with_result=False)

    print("\n=== LISTO ===")
    print("Grupos, Dieciseisavos y Octavos: simulados con resultado.")
    print("Cuartos de Final: creados en el calendario, SIN resultado — listos para que el admin los complete en vivo.")
